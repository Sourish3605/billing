import { type Invoice, type ShopSettings } from "@workspace/api-client-react";
    ...
    <div className="invoice-print-container bg-white p-8 max-w-[210mm] mx-auto text-black font-sans text-[12px] leading-tight">
      ...
      <div className="flex border print-border mb-4">
        <div className="w-1/2 p-3 border-r print-border-r">
          ...
          {invoice.customerGstin && <p className="mt-1"><span className="font-bold">GSTIN/UIN:</span> {invoice.customerGstin}</p>}
        </div>
      ...
      <table className="w-full print-table border-collapse mb-4 text-[11px]">
        <thead>
          <tr className="bg-gray-100">
            <th className="p-1.5 border print-border w-[5%]">S.No</th>
            <th className="p-1.5 border print-border w-[30%]">Description of Goods</th>
            <th className="p-1.5 border print-border w-[10%]">HSN/SAC</th>
            <th className="p-1.5 border print-border w-[8%]">Qty</th>
        ...
        <tbody>
          {invoice.items.map((item, index) => (
            <tr key={index}>
              <td className="p-1.5 border print-border text-center">{index + 1}</td>
              ...
              <td className="p-1.5 border print-border text-right">{item.rate.toFixed(2)}</td>
              <td className="p-1.5 border print-border text-right">{item.amount.toFixed(2)}</td>
              <td className="p-1.5 border print-border text-center">{item.cgstPercent}%</td>
              <td className="p-1.5 border print-border text-right">{item.cgstAmount.toFixed(2)}</td>
              <td className="p-1.5 border print-border text-center">{item.sgstPercent}%</td>
              <td className="p-1.5 border print-border text-right">{item.sgstAmount.toFixed(2)}</td>
            </tr>
          ))}
          {/* Empty rows to stretch table if needed could go here */}
        </tbody>
        <tfoot>
          <tr className="font-bold">
            ...
            <td className="p-1.5 border print-border text-right">{invoice.subTotal.toFixed(2)}</td>
            <td className="p-1.5 border print-border"></td>
            <td className="p-1.5 border print-border text-right">{invoice.totalCgst.toFixed(2)}</td>
            <td className="p-1.5 border print-border"></td>
            <td className="p-1.5 border print-border text-right">{invoice.totalSgst.toFixed(2)}</td>
          </tr>
        </tfoot>
      </table>
